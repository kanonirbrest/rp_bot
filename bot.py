...
def format_user_promo_message(code: str, created_at: str | None = None) -> str:
    ...
    return (
        ...
        "промокод в соответствующее окно👌🏻\n\n"
...
def main_menu_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Предстоящие события 📅",          callback_data="cb_events")],
        ...
        [InlineKeyboardButton("О RAZMAN production ℹ️",          callback_data="cb_about")],
    ])
...
def _offers_discounts_text() -> str:
    return (
        ...
        "3. Персональная скидка 10% при регистрации в «Клубе друзей Razman Production» "
        "в Telegram-боте (https://t.me/RazmanProductionBot)\n\n"
        "4. Повторное посещение выставки по цене льготного билета — 30 BYN. "
        "При повторном посещении необходимо показать кассиру фото, сделанное "
        "в рамках выставки «Небо.Река 2026»\n\n"
        "5. При покупке 5 и более билетов — 1 полёт на качелях включён в стоимость\n\n"
        "6. Скидка 5% на билеты в одном чеке имениннику в день его рождения, "
...
async def _event_photo(setting_key: str, fallback_url: str) -> str:
    try:
        photo = await db.get_setting(setting_key)
    except Exception:
        photo = None
    return photo or fallback_url


async def _send_events_hub(message):
    text = "Предстоящие события 📅\n\nВыбери событие:"
    await message.reply_text(text, reply_markup=events_hub_inline())
...
async def _send_announcements(message):
    await _send_events_hub(message)
...
async def cmd_events_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _send_events_hub(update.effective_message)
...
FAQ_ANSWERS = {
    ...
    "faq_gift": (
        ...
        "А также можно приобрести электронные билеты с открытой датой у нас на сайте: "
        "https://dei-tickets.onrender.com/buy-tickets-smr"
        "?utm_source=tg&utm_medium=banner&utm_campaign=podarochniesertifikaty"
    ),
    "faq_cantbuy": (
...
async def cmd_setsadyphoto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    if not update.message.photo:
...
def main():
    ...
    app.add_handler(CommandHandler("events", cmd_events_cmd))
